...
function* writeError(error) {
  yield put({
    type: authReducerTypes.setError,
    payload: {
      error: error.response.data.message || "Check internet connection",
    },
  });
}
...
function* checkEmailSaga(payload) {
  try {
    ...
    yield put({
      type: authTypes.CHECK_EMAIL_SUCCESS,
      payload: { key: "sanded" },
    });
  } catch (err) {
...
function* checkVerifyKeySaga(payload) {
  try {
    ...
    yield put({
      type: authTypes.CHECK_VERIFY_KEY_SUCCESS,
      payload: { key: "checked" },
    });
  } catch (err) {
    yield put({ type: authTypes.ERROR, payload: err });
...
function* resetPassSaga(payload) {
  try {
    ...
    /* const res = */ yield call(authService.resetPassword, {
      ...payload,
      authToken,
    });
