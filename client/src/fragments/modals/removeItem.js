...
export const RemoveItem = ({ status, close, type, id, parentName }) => {
  ...
  let Option = ({ children, status, onClick }) => (
    <div className={`option`} onClick={onClick}>
      <div className="round">{status && <div className="center" />}</div>
      <p>{children}</p>
    </div>
  );
  return (
    <BaseModal close={close} status={status} type={"remove"}>
      <Styled.Remove hasActions={hasActions}>
        ...
        {hasActions ? (
          <Styled.Options>
            <Option
              status={activeType === "one"}
              onClick={() => setActiveType("one")}
            >
              Delete from <span>{parentName}</span>{" "}
              {type === "category" ? "menu" : "category"}
            </Option>
            <Option
              status={activeType === "all"}
