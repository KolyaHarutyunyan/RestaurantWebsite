              ...
              <div className="scrolled-tab">
                ...
                >
                  ...
                  <div className="flex-able">
                    <div onClick={() => handleOpenSwipe(business, "restaurant")}>
                      {business?.logo ?
                ...
                {menus[activeTab]?.length && menus[activeTab]?.map((item, key) => (
                  ...
                  <div>
                    ...
                    <div className="category" key={key}>
                      ...
                      {item?.description && <p className="category-description">{item.description}</p>}

                      <div> {item?.items?.length && item.items.map((item, key) => (
                        item?.item?.active &&
                        ...
                        <div key={key} className="category-card" onClick={() => handleOpenSwipe(item.item, "food")}>
                          ...
                          <div className="card-info">
                            <div className="title">
                              ...
                              <p>{`$${item.item.price.toString().search("\\.") === -1 ? `${item.item.price}.00` : item.item.price}`}</p>
                            </div>
                            <p className="desc" color="text">
                              {item?.item?.description?.length > 40 ? `${item.item.description.slice(0, 40)}...` : item?.item?.description}
                            </p>
                            <p className="optional">
                              {item?.item?.note?.length > 40 ? `${item.item.note.slice(0, 40)}...` : item.item.note}
                            </p>
                          </div>
                        </div>
                      ))}
