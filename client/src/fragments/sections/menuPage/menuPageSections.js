...
export const MenuPageSections = () => {
  ...
  const getCurrentRestaurantSaga = useSagaStore(businessesActions.getCurentBusiness);
  ...
  const categoriesCheck = selectedCategory.categoryItems ? selectedCategory.categoryItems : 'NO_SELECTED'
  const categoryCheck = currentMenu[selectedCategory.categoryType]

  useEffect(async () => {
    if(currentMenu){
      if(selectedCategory.categoryId){
       const newItems = await currentMenu[selectedCategory.categoryType].find((i) => i.id === selectedCategory.categoryId && i.items)
        if(newItems) {
          setSelectedCategory({
                  ...
                  <>
                    ...
                    <div className="grid">
                      ...
                      {categoryCheck && categoryCheck.length ?
                          <Items categType={selectedCategory.categoryType}
