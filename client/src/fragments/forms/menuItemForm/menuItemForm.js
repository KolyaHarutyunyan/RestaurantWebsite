...
export const MenuItemForm = () => {
  ...
  const onSubmit = async (data) => {
    const images =
      imgPush &&
      imgPush.length &&
      (await ImgUploader(imgPush, true).then((res) => res));
    if (params.categoryItem) {
      ...
      const eventAvatar = allPhotos.length
        ? +index !== params.categoryItem.item.mainImage
          ? +index
          : params.categoryItem.item.mainImage
          ? params.categoryItem.item.mainImage
          : 0
        : 0;
