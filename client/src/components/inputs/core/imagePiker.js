...
export const ImagePiker = ({ count, brd, values, onChange, className, type, title, description }) => {
	...
	let removeItem = id => onChange(id === -1 ? "" : values.map((item, i) => id === i ? "" : item))
					...
					typeof values === "object"
						? <div className="row">
							{
								values.map((item, id) => <div
									key={id}
									style={{ cursor: "pointer",borderRadius:100,width:"fit-content" }}
									draggable={!!item}
						...
						: <div className="row">
							...
							>
								...
								/>
								<input ref={inputRefs[0]} onChange={e => fileSelectHandler(e.target.files, -1)}
											 type="file" accept={"image/*"} style={{ display: "none" }}/>
							</div>
						</div>
				}
