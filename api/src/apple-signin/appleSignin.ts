...
export class AppleSignin {
  constructor(options: AppleSigninOptions) {
    ...
    this.scope = options.scope
      ? options.scope
      : AppleSigninScope.NAME_AND_EMAIL;
   ...
   */
  authorize = async (
    auth: AppleAuthorizedResponse,
  ): Promise<AppleSignedinResponse> => {
  ...
  private validateOptions = (options: AppleSigninOptions) => {
    ...
    if (!options.privateKeyPath)
      throw new Error('PrivateKeyPath was not provided');
    if (options.scope) {
      if (
  ...
  private getAuthorizationToken = async (
    code: string,
  ...
  private refreshAuthorizationToken = async (
    refreshToken: string,
  ): Promise<AppleTokenResponse> => {
    const url = new URL(ENDPOINT_URL);
    url.pathname = '/auth/token';
